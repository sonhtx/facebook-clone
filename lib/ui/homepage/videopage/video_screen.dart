...
class VideoScreenState extends State<VideoScreen> {
  ...
  Future<void> getPost(String id) async {
    try {
      PostData? postData = await _postRepository.getPost(id);
      post = postData!;
    } catch (error) {
      print(error);
    }
                    ...
                    Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ReactionButton(
                              initialReaction:
                                  ReactionValues[int.parse(post.is_felt) + 1],
                            ...
                            _SetMarkButtonWidget(
                              can_mark: post.can_mark,
...
class _SetMarkButtonWidget extends StatelessWidget {
  const _SetMarkButtonWidget(
      {required this.can_mark, required this.id, required this.can_rate});
    ...
    return TextButtonWidget(
      ...
      onPressed: () {
        switch (can_mark) {
          ...
          case '0':
            showNotification(
                context, 'Mark status', 'Already mark, post stop allow mark');
            break;
          case '-1':
            showNotification(
                context, 'Mark status', "Can't mark your own post");
            break;
          case '-2':
            showNotification(
                context, 'Mark status', "Author've deactivated this account");
            break;
          case '-3':
            showNotification(
                context, 'Mark status', "Not mark, post stop allow mark");
            break;
          case '-4':
